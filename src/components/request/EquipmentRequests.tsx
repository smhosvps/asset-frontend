...
import {
  ...
  useGetRequestsQuery,
} from "@/redux/features/requests/requestEquipmentApi";
...
export default function MaintenanceRequest({}: Props) {
  ...
  const [cancelRequest, { isLoading: isCancelling }] =
    useCancelRequestMutation();
  const [approveERequest, { isLoading: isApproving }] =
    useApproveERequestMutation();
  ...
  const filteredRequests = maintenanceRequests.filter((request) => {
    const matchesSearch =
      ...
      formatPropertyString(getPropertyDetailsByAssetId(request.property_id))?.toLowerCase()
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      <DetailsModal
        ...
        property={
          selectedRequest
            ? getPropertyDetailsByAssetId(selectedRequest.property_id)
            : undefined
        }
        user={
          selectedRequest
            ? getUserDetailsByUserId(selectedRequest.user_id)
            : undefined
        }
      />
    </div>
